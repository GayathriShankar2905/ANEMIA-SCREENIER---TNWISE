...
class _ConjunctivaCaptureScreenState extends State<ConjunctivaCaptureScreen>
  ...
  Future<void> _autoDetectROI() async {
    ...
    try {
      ...
      for (int y = th ~/ 6; y < th * 5 ~/ 6 - bandH; y++) {
        double s = 0;
        for (int dy = 0; dy < bandH; dy++) s += smooth[y + dy];
        s /= bandH;
      ...
      Container(
        ...
        child: Column(children: [
          ...
          Container(width: 80, height: 80,
            ...
            child: Center(child: Text('👁️', style: const TextStyle(fontSize: 38)))),
          ...
          // How-to steps
          _HowToStep('1', 'Wash hands thoroughly', teal),
          const SizedBox(height: 10),
          _HowToStep('2', 'Gently pull down your lower eyelid to expose the inner surface', amber),
          const SizedBox(height: 10),
          _HowToStep('3', 'Hold under bright natural or indoor light — avoid flash', green),
          const SizedBox(height: 10),
          _HowToStep('4', 'Take a close-up photo of the exposed inner eyelid', teal),
  ...
  List<Widget> _corners(Color c) {
    ...
    return [
      Positioned(left: 0, top: 0, child: _Corner(s, t, c, [1,1])),
      Positioned(right: 0, top: 0, child: _Corner(s, t, c, [-1,1])),
      Positioned(left: 0, bottom: 0, child: _Corner(s, t, c, [1,-1])),
      Positioned(right: 0, bottom: 0, child: _Corner(s, t, c, [-1,-1])),
    ];
  }
